...
import { PROMPTS, QUICK, REASSURE } from '../../data/copy';
...
export function WriteScreen({ text, setText, onContinue }: WriteScreenProps) {
  ...
  useEffect(() => {
    const id = setInterval(() => setPromptIdx((i) => (i + 1) % PROMPTS.length), 3400);
  ...
  return (
    <div style={shell}>
      <Night moonX="50%" moonY={28} moonSize={84} fireflies={CONFIG.fireflies} fogDensity={CONFIG.fogDensity} />
      <div style={{ position: 'relative', height: '100%', display: 'flex', flexDirection: 'column', zIndex: 4 }}>
        <div style={{ flex: '0 0 auto', padding: '150px 30px 0', textAlign: 'center' }}>
          <h1
            className="cc-rise"
            style={{
              fontFamily: ccSerif,
            ...
            {!text ? (
              <Fragment>
                ...
                <div
                  ...
                  style={{ display: 'flex', gap: 8, overflowX: 'auto', padding: '0 26px 4px', margin: '0 -26px' }}
                >
                  {QUICK.map((q) => (
                    <button
                      key={q}
              ...
              >
                ...
                />
                <span style={{ fontFamily: ccS, fontSize: 13.5, color: 'var(--muted)', lineHeight: 1.4 }}>
                  {reassure || ''}
                </span>
              </div>
            )}
