...
// Set when startWind() is requested before the audio is unlocked by a gesture
// (iOS won't let us create the context yet) — we start the wind on unlock.
...
const SCALE = [
  ...
  293.66,
  329.63,
  392.0, // A3 C4 D4 E4 G4
  440.0,
  523.25,
  587.33,
  659.25,
  783.99, // A4 C5 D5 E5 G5
  880.0,
  1046.5,
  1174.66, // A5 C6 D6
];
const note = (i: number): number =>
  SCALE[Math.max(0, Math.min(SCALE.length - 1, i))];
...
export const audio = {
  ...
  startWind(): void {
    if (muted) return;
    // Don't create the AudioContext here: on mount there's been no user gesture
    // yet, and a context created outside a gesture stays muted forever on iOS.
    // Defer until the unlock handler runs on the first touch/click.
    if (!ctx) {
      pendingWind = true;
      return;
    }
...
// iOS Safari/Chrome (both WebKit) are strict: the context must be CREATED inside
// the gesture and "warmed" by playing a silent buffer, then resumed. We listen on
// several gesture types and only stop once the context is actually running.
if (typeof window !== "undefined") {
  const events = ["touchend", "pointerdown", "mousedown", "keydown", "click"];
  const unlock = () => {
    ensure(); // first call creates the context — now inside a real gesture
    if (!ctx) return;
    try {
      // A 1-sample silent buffer started in-gesture is what actually unlocks iOS.
  ...
  // iOS can suspend / "interrupt" the context when the tab is backgrounded
  // (e.g. a call, control center). Resume it when the user comes back.
  document.addEventListener("visibilitychange", () => {
    if (!document.hidden && ctx && ctx.state !== "running" && !muted)
      void ctx.resume();
  });
}
